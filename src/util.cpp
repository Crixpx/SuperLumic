...
const char * const BITCOIN_CONF_FILENAME = "superlumic.conf";
const char * const BITCOIN_PID_FILENAME = "superlumicd.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "superlumic" is a composite category enabling all SuperLumic-related debug output
            if(ptrCategory->count(string("superlumic"))) {
...
#else
    const char* pszModule = "superlumic";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\SuperLumicCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\SuperLumicCore
    // Mac: ~/Library/Application Support/SuperLumicCore
    // Unix: ~/.superlumiccore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "SuperLumicCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/SuperLumicCore";
#else
    // Unix
    return pathRet / ".superlumiccore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty superlumic.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override superlumic.conf
